+++
title = 'References'
description = 'Official Hugo documentation used by this example.'
weight = 100
toc = true
+++

These are the official sources behind the patterns in this site. Start with the
guide for a working example, then use the reference for full behavior and edge
cases.

## Project and content

- [Directory structure](https://gohugo.io/getting-started/directory-structure/)
- [Configuration](https://gohugo.io/configuration/)
- [Content management](https://gohugo.io/content-management/)
- [Front matter](https://gohugo.io/content-management/front-matter/)
- [Page bundles](https://gohugo.io/content-management/page-bundles/)
- [Page resources](https://gohugo.io/content-management/page-resources/)
- [Archetypes](https://gohugo.io/content-management/archetypes/)
- [Taxonomies](https://gohugo.io/content-management/taxonomies/)
- [Menus](https://gohugo.io/content-management/menus/)
- [Multilingual sites](https://gohugo.io/content-management/multilingual/)

## Templates and output

- [Template introduction](https://gohugo.io/templates/introduction/)
- [Lookup order](https://gohugo.io/templates/lookup-order/)
- [Render hooks](https://gohugo.io/render-hooks/)
- [Shortcodes](https://gohugo.io/content-management/shortcodes/)
- [Pagination](https://gohugo.io/templates/pagination/)
- [Output formats](https://gohugo.io/configuration/output-formats/)
- [RSS templates](https://gohugo.io/templates/rss/)
- [Sitemap templates](https://gohugo.io/templates/sitemap/)
- [404 templates](https://gohugo.io/templates/404/)

## Assets and data

- [Resource functions](https://gohugo.io/functions/resources/)
- [Image processing](https://gohugo.io/content-management/image-processing/)
- [Hugo Pipes](https://gohugo.io/hugo-pipes/introduction/)
- [Data sources](https://gohugo.io/content-management/data-sources/)

## Build and deploy

- [Hugo command](https://gohugo.io/commands/hugo/)
- [Host and deploy](https://gohugo.io/host-and-deploy/)
